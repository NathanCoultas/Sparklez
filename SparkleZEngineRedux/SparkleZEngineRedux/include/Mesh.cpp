...
namespace sparklezEngine
{
	...
	Mesh::Mesh()
	{
		...
		LoadObj("include/Meshs/Cube.obj");
	}
	...
	void Mesh::LoadObj(std::string _Filename)
	{
			...
			while (std::getline(inputFile, currentLine))
			{
				...
				else if (!currentLine.substr(0, 2).compare(0, 1, "f"))
				{
					...
					if (verts[3].empty())
					{
						...
						m_OrderedVerts = orderedPositionData;
						m_OrderedNormals = orderedNormalData;
						m_OrderedUV = orderedUVData;;
					}
					else
					{
						Console::ErrorMessage("WARNING: This OBJ loader only works with triangles but a quad has been detected. Please triangulate the mesh.");
						Console::ErrorMessage(_Filename);
						inputFile.close();
						return;
					}
		...
		{
			Console::ErrorMessage("WARNING: File not found");
			Console::ErrorMessage(_Filename);
			m_MeshFileName = "NULL";
		}
	}
